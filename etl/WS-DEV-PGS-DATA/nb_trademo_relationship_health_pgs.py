...
# Target: trademo.relationship_health  (localhost:5432/goglo_etl)
...
try:
    ...
    existing_keys = (
        ...
        .option("url", PG_URL)
        .option("dbtable", "(SELECT rh_key FROM trademo.relationship_health) AS t")
    ...
    if new_count > 0:
        new_rows.write.format("jdbc") \
            .option("url", PG_URL) \
            .option("dbtable", "trademo.relationship_health") \
            .option("user", PG_PROPS["user"]) \
            .option("password", PG_PROPS["password"]) \
            .option("driver", PG_PROPS["driver"]) \
            .mode("append") \
            .save()
        logger.info(f"Inserted {new_count} rows into trademo.relationship_health")
    else:
