...
#!/usr/bin/env python3
# --------------------------------------------------------------------------------------
# ZoomInfo Scoop Search — PGS (Gold → PostgreSQL goglo_etl)
# Target: zoominfo.scoop_search  (localhost:5432/goglo_etl)
# --------------------------------------------------------------------------------------
...
logger.info("Starting Scoop Search Gold → PostgreSQL load")

try:
    df_gold = spark.read.format("delta").load(gold_path)

    df_out = df_gold.select(
        col("sc_key"), col("ingested_at"), col("scoop_id"), col("scoop_title"),
        col("scoop_summary"), col("scoop_type"), col("scoop_topic"), col("company_id"),
        col("company_name"), col("published_date"), col("scoop_url"), col("scoop_source"),
    ...
    existing_keys = (
        ...
        .option("url", PG_URL)
        .option("dbtable", "(SELECT sc_key FROM zoominfo.scoop_search) AS t")
        .option("user", PG_PROPS["user"]).option("password", PG_PROPS["password"])
        .option("driver", PG_PROPS["driver"]).load()
    )

    new_rows  = df_out.join(existing_keys, on="sc_key", how="left_anti")
    new_count = new_rows.count()
    logger.info(f"New rows to insert: {new_count}")

    if new_count > 0:
        new_rows.write.format("jdbc") \
            .option("url", PG_URL).option("dbtable", "zoominfo.scoop_search") \
            .option("user", PG_PROPS["user"]).option("password", PG_PROPS["password"]) \
            .option("driver", PG_PROPS["driver"]).mode("append").save()
        logger.info(f"Inserted {new_count} rows into zoominfo.scoop_search")
    else:
...
logger.info("Scoop Search PGS load complete.")
