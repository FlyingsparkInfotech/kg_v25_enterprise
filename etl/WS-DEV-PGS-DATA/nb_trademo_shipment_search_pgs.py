#!/usr/bin/env python3
"""
Trademo Shipment Search — Postgres Load
WS-DEV-PGS-DATA/nb_trademo_shipment_search_pgs.py

Anti-joins the gold Delta table against raw.trademo_shipment_search in
Postgres on shipment_key, then JDBC-appends only new rows to that table.

Target table : raw.trademo_shipment_search
...
PGS_TABLE = "raw.trademo_shipment_search"
...
def build_spark() -> SparkSession:
    return (
        ...
        .appName("nb_trademo_shipment_search_pgs")
        .config(
            "spark.jars.packages",
            ",".join([
...
def read_pgs_keys(spark: SparkSession):
    """Return a DataFrame of shipment_key values already in Postgres."""
    return spark.read.jdbc(
        url=PGS_URL,
        table=f"(SELECT shipment_key FROM {PGS_TABLE}) AS pgs_keys",
...
def run(spark: SparkSession) -> None:
    ...
    log.info("Reading existing shipment_key values from %s", PGS_TABLE)
    try:
...
def main() -> None:
    ...
    log.info("nb_trademo_shipment_search_pgs.py — done.")
