#!/usr/bin/env python3
"""
Trademo Shipment Search — Gold Layer
WS-DEV-GOLD-DATA/nb_trademo_shipment_search_gold.py

Assigns a gap-free sequential surrogate key (shipment_key) to shipments
that do not yet exist in the gold table (anti-join on shipment_id), then
appends them to the gold Delta table.

Surrogate key : shipment_key  (Window.orderBy(shipment_id), sequential, gap-free)
Chains to     : nb_trademo_shipment_search_pgs.py
"""
...
S3_GOLD   = "s3a://goglo-gold-layer/trademo/trademo-etl-shipment-search-gold"

NEXT_SCRIPT = os.path.join(
    ETL_BASE,
    "WS-DEV-PGS-DATA",
    "nb_trademo_shipment_search_pgs.py",
)
...
def build_spark() -> SparkSession:
    return (
        ...
        .appName("nb_trademo_shipment_search_gold")
        .config(
            "spark.jars.packages",
            ",".join([
...
def run(spark: SparkSession) -> None:
    ...
    log.info(
        "New records to add to gold: %d  (current max shipment_key=%d)",
        record_count,
        max_key,
    )
...
def main() -> None:
    ...
    chain_next()
    log.info("nb_trademo_shipment_search_gold.py — done.")
