...
#!/usr/bin/env python3
# --------------------------------------------------------------------------------------
# Trademo Company Matcher — GOLD (surrogate key cm_key + incremental merge)
# Merge key: company_id
# Chains to WS-DEV-PGS-DATA/nb_trademo_company_matcher_pgs.py
# --------------------------------------------------------------------------------------
...
logger.info("Starting Company Matcher Silver → Gold")
...
try:
    ...
    if gold_exists:
        ...
        max_key   = df_gold.select(F.max("cm_key")).collect()[0][0]
        start_key = (max_key + 1) if max_key is not None else 1
    else:
        df_gold   = None
        start_key = 1

    logger.info(f"Next cm_key starts at: {start_key}")
    ...
    if new_count > 0:
        ...
        df_new_with_key = df_new.withColumn(
            "cm_key", row_number().over(window_spec) + (start_key - 1)
        )
        ordered = ["cm_key"] + [c for c in df_new_with_key.columns if c != "cm_key"]
    ...
    if not gold_exists:
        ...
        df_matched = df_silver.join(
            df_gold.select("company_id", "cm_key"), on="company_id", how="inner"
        )
...
subprocess.run(
    ["python3", f"{ETL_BASE}/WS-DEV-PGS-DATA/nb_trademo_company_matcher_pgs.py"],
    check=True
)
