...
#!/usr/bin/env python3
# --------------------------------------------------------------------------------------
# ZoomInfo Scoop Search — SILVER (Delta merge)
# Merge key: scoop_id
# Chains to WS-DEV-GOLD-DATA/nb_zoominfo_scoop_search_gold.py
# --------------------------------------------------------------------------------------
...
logger.info("Starting Scoop Search RAW → Silver")
...
try:
    ...
    # Dedup: keep latest per scoop_id
    window_spec = Window.partitionBy("scoop_id").orderBy(col("ingested_at").desc())
    df_dedup = (
        df_transformed
    ...
    if not silver_exists:
        ...
        (
            dt.alias("t")
            .merge(df_dedup.alias("s"), "t.scoop_id = s.scoop_id")
...
subprocess.run(
    ["python3", f"{ETL_BASE}/WS-DEV-GOLD-DATA/nb_zoominfo_scoop_search_gold.py"],
    check=True
)
